...
| `fault` | `null` або `{"code":"DPS_TIMEOUT","since":91100,"detail":"..."}` |
...
Коди відмов: `DPS_TIMEOUT`, `DPS_PROTECT`, `DPS_STATE_MISMATCH`, `RELAY_STUCK`, `T_OVER`. `OC_LATCH` зарезервований для сумісності, але не генерується триконтактною ревізією mACS: входу FAULT немає.
